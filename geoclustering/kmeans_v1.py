...
file = (type + 'Final_one_band_'
        + gr_row + '_'
        + gr_col + '_'
        + clus + '_'
        + iter + '_new.tif')
root_image_cc = root + file
file_csv = (type + 'Final_one_band_'
            + gr_row + '_'
            + gr_col + '_'
            + clus + '_'
            + iter + '_new.csv')
...
# Statistics measures (Mean, STD, 5 percentile, 95 percentile, maximum and
# minimum values) from each co-cluster group:
...
for r in gr_rows:
    for c in range(0, C.shape[1]):
        ...
        data = np.array([np.nanmean(Index_temp), np.nanstd(Index_temp),
                         np.nanpercentile(Index_temp, 5),
                         np.nanpercentile(Index_temp, 95),
                         np.nanmax(Index_temp), np.nanmin(Index_temp)])
...
# # Obtain the centroids (mean values)
# of each group and they are transformed to matrix:
mean_centroids = (kmeans_cc.cluster_centers_[:, 0] * (X[1][0][1] - X[1][0][0])
                  + X[1][0][0])
...
plt.savefig(rootOutput + 'Spatial_info.png', format='png', transparent=True,
            bbox_inches="tight")
plt.close(fig)
...
plt.savefig(rootOutput + 'Temporal_info.png', format='png', transparent=True,
            bbox_inches="tight")
